                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('keranjang.updateJumlah', $keranjang->id) }}" method="post" class="d-flex align-items-center justify-content-center" id="form-keranjang-{{ $keranjang->id }}">
                                            @csrf
                                            @method('PATCH')
                                            ...
                                            <input type="number" name="jumlah" id="jumlah-{{ $keranjang->id }}" value="{{ $keranjang->jumlah }}" class="form-control mx-2 text-center" style="width: 70px;" min="1" max="{{ $keranjang->produk->stockproduk->stock }}" readonly>

                                            <!-- Tombol untuk menambah jumlah -->
                                            <button type="button" class="btn btn-sm btn-outline-secondary" id="btn-plus-{{ $keranjang->id }}" @if($keranjang->jumlah >= $keranjang->produk->stockproduk->stock) disabled @endif>+</button>
                                        </form>
                                    </td>
                                    ...
                                    <script>
                                        ...
                                        document.getElementById('btn-plus-{{ $keranjang->id }}').addEventListener('click', function() {
                                            ...
                                            if (jumlah < {{ $keranjang->produk->stockproduk->stock }}) {
